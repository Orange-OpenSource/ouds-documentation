...
The clean-diff "new images only" Phase C skipped `badge@spec_layout_spacing` even though it was white-filled in Phase B → the white background never reached the repo. Re-exported `badge@spec_layout_spacing.png` (frame fill confirmed `#ffffff`). **Skill updated:** Phase C now *mandatorily* re-exports `@spec_layout_spacing` on every run, after an idempotent `#ffffff` fill check.
