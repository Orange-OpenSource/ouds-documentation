...
**Build-time metrics vs runtime tracking.** Pour les dashboards d'adoption, les métriques build-time (imports, drift counts, bypass signals, PR delta) sont plus utiles et moins invasives que le tracking runtime. Elles décrivent ce qui se passe dans le code sans surveiller les personnes. Enrichit [[metriques-adoption-ia-design-system]].
...
- Confirme et enrichit [[architecture-skills-rules-instructions]] avec des noms de Skills concrets et des templates de custom Skills
- Introduit [[audit-tokens-playwright]] : pattern Planner/Generator/Healer, premier workflow d'audit continu dans le corpus
- Introduit [[bypass-patterns-comme-user-research]] : renversement paradigmatique sur l'interprétation des bypasses
- Introduit [[assistant-ia-24h]] : OpenClaw comme infrastructure d'équipe persistente
- Confirme [[documentation-lag]] et enrichit [[pipeline-figma-docs-automatise]] avec le cron job de re-sync
- Enrichit [[metriques-adoption-ia-design-system]] : build-time signals comme substituts aux métriques runtime invasives
- S'aligne avec [[mcp-model-context-protocol]] : Figma MCP, GitHub MCP, Slack MCP, Notion MCP, PostHog MCP, Zapier MCP listés comme connecteurs clés
