...
Sa formulation : "If a better tool shows up tomorrow, you swap the center, and everything else stays the same. That's the point of building on a protocol, not a product" ([[self-healing-design-system]]).
...
Cette logique est exactement celle que [[mcp-model-context-protocol]] décrit à un niveau plus générique ("USB pour l'IA") — mais Kavcic l'applique ici à la décision architecturale concrète : ne jamais construire une intégration sur l'API propriétaire d'un outil IA quand une couche MCP standard est disponible.
