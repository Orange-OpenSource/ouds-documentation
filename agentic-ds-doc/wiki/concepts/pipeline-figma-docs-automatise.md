...
Le pipeline Figma → docs automatisé est la réponse architecturale au [[documentation-lag]] décrite par [[romina-kavcic]] ([[how-to-automate-design-system-documentation]]). Il connecte trois systèmes — Figma (via API et MCP), un outil IA (Claude Code), et [[mintlify]] (hébergement de docs) — de sorte que les mises à jour de design se propagent automatiquement à la documentation sans intervention manuelle.
...
Ce mécanisme de tracking est une forme de métadonnée de provenance : la documentation sait d'où elle vient. Sans ce lien, l'automatisation ne peut pas savoir quoi vérifier. C'est l'équivalent documentaire de l'index de codebase ([[knowledge-graph-design-system]]) : on encode explicitement les relations entre artefacts pour que les agents puissent naviguer sans explorer.

## Les writing guidelines comme couche de cohérence

Pour des résultats consistants entre les différentes mises à jour générées par Claude Code, [[romina-kavcic]] recommande de créer un fichier Markdown de writing guidelines : ton, voix, règles de rédaction, exemples de dos/don'ts. Ce fichier joue le même rôle que les Rules dans l'architecture [[architecture-skills-rules-instructions]] : un contexte always-on qui garantit la cohérence stylistique de toutes les sorties, indépendamment du composant traité.

## Relation avec le pipeline de métadonnées

Ce pipeline est une variante du [[processus-generation-metadata-echelle]] appliqué à la documentation narrative plutôt qu'aux métadonnées structurées machines. Les deux s'appuient sur la même logique — automatiser l'extraction depuis les sources existantes plutôt que produire manuellement — mais leurs sorties sont différentes : le pipeline de métadonnées produit du JSON pour les agents IA, le pipeline Figma → docs produit du MDX pour les humains (et potentiellement les agents).

## Variante Uber : génération de spec formelle (uSpec / Guisard)

[[ian-guisard]] chez Uber documente une variante architecturalement différente des pipelines précédents ([[uber-uspec-agentic-design-specs]]) : la sortie n'est pas de la documentation narrative (MDX, Mintlify) mais des **pages de spec formelles** écrites directement dans Figma — anatomie du composant, documentation API, annotation de tokens, règles d'accessibilité multi-plateforme. Voir [[generation-spec-agentique]] pour le détail complet.
...
[[fernando-coelho]] chez Typeform documente une variante architecturalement différente ([[typeform-design-system-documentation-scale-ai]]) : la source n'est pas Figma mais Notion, et le pipeline alimente non pas une seule destination mais trois simultanément (documentation, MCP, sandbox). Voir [[pipeline-multi-destinations]] pour le détail complet.
...
Le fichier OUDS Actions---Button ([[figma-ouds-button-specs]]) illustre une limite architecturale du pipeline qui n'était pas documentée dans le corpus. Le frame de documentation principal (node 2071:11587) mesure 31 742 × 13 443 px et contient plusieurs colonnes de specs pour toutes les variantes du composant. Ce frame génère plus de 2,3 millions de caractères XML via l'API Figma — une taille qui dépasse la capacité de traitement en un seul appel MCP.
