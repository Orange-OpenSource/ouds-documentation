...
[[romina-kavcic]] liste ce qu'un fichier de tokens doit répondre pour être agent-ready :
...
La différence entre deviner et savoir est un seul champ par token. Ce n'est pas une question de modèle, de prompt ou d'architecture — c'est une question de données disponibles dans le fichier. Voir [[dtcg-annotation-schema]] pour le template concret.
...
Le nom *laisse entendre* un intent. Le fichier garde le silence sur le rôle, l'usage, ce qu'il faut éviter. L'agent obtient une valeur et infère le reste — avec toute la variance que l'inférence implique ([[priori-conflictuels-nommage]]).

## Le lien avec la gouvernance des violations

La distinction readable/usable recoupe directement [[existence-vs-intent-violations]]. Un fichier readable sans descriptions permet uniquement l'audit v1 (vérifier que les tokens référencés existent). Pour détecter les violations d'intent — un token utilisé dans un contexte sémantiquement incorrect — l'auditeur a besoin que le fichier dise ce que chaque token *devrait* faire. Sans la couche usable, la gouvernance reste au niveau linter, jamais au niveau sémantique.

## Confirmation empirique sur les icônes (Miro)

[[eddie-machado]] chez Miro documente le même problème étendu aux icônes, avec un cas concret de production ([[miro-ai-design-system-mcp-claude-code-skills]]). Une icône visuellement représentant "two A's, one capital, one cursive" était nommée dans la librairie comme si elle servait les "text styles" alors qu'elle sert les "font styles" (serif, sans-serif). L'icône pour les text styles réels est nommée d'après sa forme visuelle. Résultat : Aura était "confidently wrong" — elle construisait des toolbars avec les mauvaises icônes, avec confiance.
...
La réponse de Miro : trois champs de métadonnées par icône (description visuelle, cas d'usage, catégorie) + règle de non-usage explicite. Après ajout : les mêmes prompts produisent des sorties correctes. Voir [[schema-metadata-composant]] pour le schéma complet.
