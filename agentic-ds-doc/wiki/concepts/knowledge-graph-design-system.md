...
[cristian-morales-achiardi](../entities/cristian-morales-achiardi.md) formule la complémentarité entre métadonnées et index de manière précise ([mapping-design-system-for-ai-agents](../sources/mapping-design-system-for-ai-agents.md)) : les métadonnées répondent à "comment utiliser Button ?" (consulter `Button.metadata.ts`), l'index répond à "où Button est-il utilisé ?" (consulter `component-usage.toon`), et les deux ensemble permettent de répondre à "devrait-on créer un nouveau composant card ?" (vérifier l'index pour les cards existantes, les métadonnées pour leurs capacités). Aucune des deux couches seule ne suffit pour cette troisième question.
