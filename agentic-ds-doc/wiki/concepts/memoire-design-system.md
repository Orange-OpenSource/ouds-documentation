...
Les données économiques sont convergentes : Uber a dépassé son budget IA, Nvidia déclare que le compute coûte plus que les salariés, Gartner documente que les systèmes agentiques consomment 5 à 30 fois plus de tokens qu'un chatbot standard (voir [[gouvernance-design-system-ia]] pour le détail). La baisse du coût unitaire des tokens ne compense pas une consommation qui croît plus vite que les économies.

## La distinction : avoir vs rendre retrievable

La mémoire institutionnelle existe dans la plupart des équipes en phase 1→100 ([[design-system-comme-dataset]]) : trois ans de renommages documentés, une douzaine de deprecations et leurs raisons, des critiques récurrentes, des rapports de drift, des ADRs. Le problème n'est pas l'absence de mémoire. C'est que cette mémoire n'est pas *retrievable* avant que l'agent commence à agir.
